...
@MultipartConfig()
@WebServlet("/email/kolEmail.do")
public class KolEmailServlet extends HttpServlet {
...
//					System.out.println("email_num="+email_num+"|"+"email_type="+email_type);
				...
				String url = "/kolBackStage/email/trashCan.jsp";
				...
				RequestDispatcher failureView = req
						.getRequestDispatcher("/kolBackStage/email/trashCan.jsp");
		...
		if ("reply".equals(action)) { // 回信
			...
			try {
				...
				String url = "/kolBackStage/email/sendEmail.jsp";
				RequestDispatcher successView = req.getRequestDispatcher(url);
				successView.forward(req, res);
				...
				RequestDispatcher failureView = req
						.getRequestDispatcher("/kolBackStage/email/Email.jsp");
				failureView.forward(req, res);
			}
		}
		...
		if ("draftSend".equals(action)) { // 回信
			...
			try {
				...
				errorMsgs.add("無法取得修改的資料:");
				RequestDispatcher failureView = req
						.getRequestDispatcher("/kolBackStage/email/Email.jsp啥");
				failureView.forward(req, res);
			}
		}
		...
		if ("showletter".equals(action)) { 
			...
			try {
				...
				/***************************3.查詢完成,準備轉交(Send the Success view)************/
				req.setAttribute("emailDetailVO", emailDetailVO);
				String url = "/kolBackStage/email/letter.jsp";
				RequestDispatcher successView = req.getRequestDispatcher(url);
				successView.forward(req, res);
        ...
        if ("insert".equals(action)) { 
			...
			try {
				...
				emailDetailVO.setSender(kolMebVO.getKol_account());//寄件人
				
				if (!errorMsgs.isEmpty()) {
					RequestDispatcher failureView = req
							.getRequestDispatcher("/kolBackStage/email/sendEmail.jsp");
					failureView.forward(req, res);
					return;
				}
				...
				String url = "/kolBackStage/email/Email.jsp";
				RequestDispatcher successView = req.getRequestDispatcher(url); 
				successView.forward(req, res);				
