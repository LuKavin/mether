...
@MultipartConfig()
@WebServlet("/email/admemail.do")
public class AdmEmailServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest req, HttpServletResponse res)
		...
		KolMebVO kolMebVO =(KolMebVO)req.getSession().getAttribute("kolMebVO");//登入者的資料
		...
		if ("rollBack".equals(action)) { // 回信
			...
			try {
				...
				String url = "/backStage/email/trashCan.jsp";
				...
				RequestDispatcher failureView = req
						.getRequestDispatcher("/backStage/email/trashCan.jsp");
		...
		if ("reply".equals(action)) { // 回信
			...
			try {
				...
				String url = "/backStage/email/sendEmail.jsp";
				RequestDispatcher successView = req.getRequestDispatcher(url);
				successView.forward(req, res);
				...
				RequestDispatcher failureView = req
						.getRequestDispatcher("/backStage/email/Email.jsp");
				failureView.forward(req, res);
			}
		}
		
		if ("draftSend".equals(action)) { 
			List<String> errorMsgs = new LinkedList<String>();
			req.setAttribute("errorMsgs", errorMsgs);
			...
			try {
				...
				errorMsgs.add("無法取得修改的資料:");
				RequestDispatcher failureView = req
						.getRequestDispatcher("/backStage/email/Email.jsp啥");
				failureView.forward(req, res);
			}
		}
		...
		if ("showletter".equals(action)) { 
			...
			try {
				...
				/***************************3.查詢完成,準備轉交(Send the Success view)************/
				req.setAttribute("emailDetailVO", emailDetailVO);
				String url = "/backStage/email/letter.jsp";
				RequestDispatcher successView = req.getRequestDispatcher(url);
				successView.forward(req, res);
        ...
        if ("insert".equals(action)) { 
			...
			try {
				...
				Integer email_typenum = 4;//信件類別,管理員記的信為系統信件=4
				...
				emailDetailVO.setSender("ADM");//寄件人
				
				if (!errorMsgs.isEmpty()) {
					RequestDispatcher failureView = req
							.getRequestDispatcher("/backStage/email/sendEmail.jsp");
					failureView.forward(req, res);
					return;
				}
				...
				String url = "/backStage/email/Email.jsp";
				RequestDispatcher successView = req.getRequestDispatcher(url); 
				successView.forward(req, res);				
				...
				emailDetailService.sendErrorLetter(kolMebVO.getKol_account(),kolMebVO.getMeb_accessnum(),mem_account);
				RequestDispatcher failureView = req
						.getRequestDispatcher("/backStage/email/Email.jsp");
				failureView.forward(req, res);
			} catch (Exception e) {
        ...
        if ("draft".equals(action)) { 
			...
			try {
				...
				String mem_account = kolMebVO.getKol_account();//收件人,草稿信收件人為自己
				Integer mem_access = emailDetailService.findMemAccess(mem_account);//判斷收件者的類型1=廠商,2=網紅
