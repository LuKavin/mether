...
@WebServlet("/message/comMessage.do")
@MultipartConfig()
public class MessageDetailController extends HttpServlet {
	...
	public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		try {
			...
			messageDetailVO.setCom_message(message);
			...
			messageDetailService.comAddMessage(messageDetailVO);

			/*************************** 3.新增完成,準備轉交(Send the Success view) ***********/

			if("製作中".equals(order_status)) {
				req.getRequestDispatcher("/comBackStage/order/orderComTurnOne.jsp").forward(req, res); // 新增成功後轉交companyPhotoIndexJsp2.jsp
			}else if("審核中".equals(order_status)) {
				req.getRequestDispatcher("/comBackStage/order/orderComTurnTwo.jsp").forward(req, res); // 新增成功後轉交companyPhotoIndexJsp2.jsp
			}
